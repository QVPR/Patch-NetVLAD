...
def download_all_models(ask_for_permission=False):
    ...
    if not ask_for_permission or ask_yesno("Auto-download pretrained models into " + dest_dir + " (takes around 2GB of space)? Yes/no."):
        if not os.path.join(dest_dir, "mapillary_WPCA128.pth.tar"):
        ...
        if not os.path.join(dest_dir, "mapillary_WPCA512.pth.tar"):
        ...
        if not os.path.join(dest_dir, "mapillary_WPCA4096.pth.tar"):
        ...
        if not os.path.join(dest_dir, "pittsburgh_WPCA128.pth.tar"):
        ...
        if not os.path.join(dest_dir, "pittsburgh_WPCA512.pth.tar"):
        ...
        if not os.path.join(dest_dir, "pittsburgh_WPCA4096.pth.tar"):
